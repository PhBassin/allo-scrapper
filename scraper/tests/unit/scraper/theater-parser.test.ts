...
import { parseTheaterPage } from '../../../src/scraper/theater-page-parser.js';
...
describe('parseTheaterPage - Edge Cases', () => {
  ...
  it('should handle theater with no movies showing', () => {
    ...
    expect(result.theater.id).toBe('NOFILMS');
...
describe('parseTheaterPage - Data Validation', () => {
  ...
  it('should parse valid film IDs for all theaters', () => {
  ...
  it('should generate deterministic showtime IDs based on stable business fields', () => {
    // IDs must be deterministic: same theater+film+date+time+version+format always → same ID
