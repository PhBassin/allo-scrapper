...
## 2026-05-31 - [Avoid Map and array methods in hot paths]
**Learning:** In performance-critical loops like `mergeMoviesAndShowtimes` and `getWeeklyMovies` that process arrays of objects, using `Map` along with array methods (`.map()`, spread operators, and `Array.from(Map.values())`) introduces measurable overhead due to prototype lookups and memory allocation (intermediate array closures).
**Action:** Use `Object.create(null)` for plain key-value dictionaries, maintain parallel arrays for ordered insertion, and use standard `for` loops with `Object.assign` to minimize overhead and allocations in data transformation bottlenecks.
