...
## 2026-03-26 - Authorization Bypass Modifying System Roles
**Vulnerability:** System roles (like the `admin` role) could be modified (both their name/description and their permissions) via the `PUT /api/roles/:id` and `PUT /api/roles/:id/permissions` endpoints. While system roles could not be deleted, they could be altered, potentially allowing privilege escalation or disruption.
**Learning:** The deletion logic correctly checked `if (role.is_system)` but this check was missing from the update logic. Because of this, an authorized user with `roles:update` permission could modify system roles that should be immutable via the API.
**Prevention:** Always verify the `is_system` attribute (or similar immutability flags) consistently across all mutation endpoints (UPDATE, DELETE), not just DELETE.
