...
## 2026-05-14 - Fix rate-limit token bucketing vulnerability
**Vulnerability:** The application used `jwt.decode` instead of `jwt.verify` to extract user IDs from tokens for rate-limiting purposes.
**Learning:** This allowed an attacker to craft an unverified JWT with another user's ID, successfully falling into their rate-limit bucket and intentionally exhausting their limits, leading to a targeted Denial of Service (DoS) attack.
**Prevention:** Always validate the token signature using `jwt.verify` before trusting the payload values, even for non-authentication operations like rate-limiting bucketing. Make sure to wrap it in a `try/catch` block.
