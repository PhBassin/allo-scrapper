...
## 2026-05-20 - Insecure JWT Rate Limit Bucketing
**Vulnerability:** The rate-limiting middleware (`server/src/middleware/rate-limit.ts`) used `jwt.decode` instead of `jwt.verify` to extract user IDs from incoming Authorization headers for the purpose of request bucketing. Since `jwt.decode` ignores the token signature, an attacker could craft an invalid, spoofed JWT containing another user's ID and exhaust their rate limit quota, resulting in a targeted Denial of Service (DoS).
**Learning:** Developers sometimes mistake `jwt.decode` as a lightweight or safe alternative when they "just need the payload", forgetting that without signature verification, the payload is completely untrusted user input.
**Prevention:** Always use `jwt.verify` when extracting security-critical identifiers (like user IDs) from tokens, even for seemingly secondary mechanisms like rate limiting. If verification fails, safely fall back to IP-based tracking.
