...
## 2024-06-14 - System Role Immutability Enforcement
**Vulnerability:** Business logic/authorization bypass allowed the modification of system roles (e.g., 'admin') via `PUT /api/roles/:id` and `PUT /api/roles/:id/permissions` endpoints, which could lead to privilege escalation or denial of service by tampering with critical permissions.
**Learning:** While the `DELETE` endpoint correctly verified the `is_system` flag to prevent deletion, the update and permission modification endpoints were missing this check. Database-level queries (e.g., `updateRole`, `setRolePermissions`) do not inherently enforce business logic constraints like `is_system`.
**Prevention:** Always consistently apply state immutability checks across all mutation endpoints (`PUT`, `PATCH`, `DELETE`) for entities with system-level protection flags. Enforce these checks at the route handler level before executing database operations.
