...
```sql
INSERT INTO permissions (name, description, category) VALUES
  ('theaters:read', 'Voir la liste des cinémas', 'theaters'),
  ('users:read',   'Voir les détails utilisateur', 'users');
```

**Rationale:**

These permissions were being checked in client code (`TheaterPage.tsx`, permission tests) but didn't exist in the database canonical permission list, creating inconsistency. Rather than removing these checks, we added the permissions to support fine-grained read access separate from write permissions.
