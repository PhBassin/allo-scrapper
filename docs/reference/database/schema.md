...
Stores theater information scraped from external sources.
...
SELECT s.time, m.title, s.version, s.format
FROM showtimes s
...
SELECT t.name AS theater, s.date, s.time, s.version
FROM showtimes s
...
SELECT m.title, t.name, s.date, s.time
FROM showtimes s
JOIN movies m ON s.movie_id = m.id
JOIN theaters t ON s.theater_id = t.id
...
FROM weekly_programs wp
JOIN theaters t ON wp.theater_id = t.id
...
WHERE wp.week_start = '2026-03-10'
  AND wp.is_new_this_week = 1
ORDER BY t.city, t.name, m.title;
```
...
WHERE s.movie_id = 12345
  AND s.date >= CURRENT_DATE
GROUP BY t.id, t.name, t.city, t.postal_code
