...
--   {cinema_id}_{movie_id}_{date}_{time}_{version}_{format}
...
--   2. Adds a UNIQUE constraint on (cinema_id, movie_id, date, time, version, format)
...
-- A "duplicate" is defined as same (cinema_id, movie_id, date, time, version, format).
DELETE FROM showtimes
WHERE id NOT IN (
  SELECT MIN(id)
  FROM showtimes
  GROUP BY cinema_id, movie_id, date, time, version, COALESCE(format, '')
);
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE showtimes
      ...
      UNIQUE (cinema_id, movie_id, date, time, version, format);
...
-- SELECT cinema_id, movie_id, date, time, version, format, COUNT(*) as cnt
--   FROM showtimes
--   GROUP BY cinema_id, movie_id, date, time, version, format
--   HAVING COUNT(*) > 1;
