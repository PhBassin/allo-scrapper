 ...
 * a theater-specific scrape operation (only that theater, all movies, all dates).
 */
...
test.describe('Theater Page - Theater-Specific Scrape', () => {
  ...
  test('theater page displays theater-specific scrape button', async ({ page }) => {
    ...
    const theaterScrapeButton = page.locator('button').filter({ hasText: /scraper uniquement ce theater/i });
  ...
  test('theater scrape button is positioned as sticky at top-20', async ({ page }) => {
    ...
    const buttonContainer = page.locator('button').filter({ hasText: /scraper uniquement ce theater/i }).locator('..');
